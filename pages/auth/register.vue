<template>
  <view class="register-container">
    ...
    <view class="login-card">
      ...
      <!-- 出生日期 -->
      <view class="input-group" :class="{'input-error': birthDateError}">
        <image 
          src="/static/auth/carlendar-icon.png" 
          class="input-icon"
          mode="aspectFit"
        />
        <picker 
        ...
        >
          <view class="picker" :class="{ placeholder: !form.birthDate }">
            {{ form.birthDate || '请选择出生日期' }}
          </view>
        </picker>
      </view>
      <view class="error-msg">{{ birthDateError }}</view>
      ...
      <view class="login-link">
        <text>已有账号?</text>
        <text class="login-text" @click="navigateTo('/pages/auth/login')">立即登录</text>
      </view>
    </view>
...
const maxDate = ref(new Date().toISOString().split('T')[0]) // 今天作为最大日期
...
const getVerificationCode = async () => {
  ...
  try {
    ...
    // 调用发送验证码API
    await sendSmsCode(form.value.phone, 1) // 1-注册验证码
...
const handleRegister = async () => {
  ...
  try {
    ...
    // 注册成功后自动登录
     tokenStore.setUser(response)  // 修改这里
