...
import { sendDoctorSmsCode, changeDoctorPasswordBySms } from '@/api/doctor'
...
const getVerificationCode = async () => {
  ...
  try {
    ...
    await sendDoctorSmsCode(form.value.phone, 3)  // 3-重置密码
    uni.hideLoading()
...
const handleResetPassword = async () => {
  ...
  try {
    ...
    await changeDoctorPasswordBySms(
      form.value.phone,
      form.value.code,
...
const navigateToLogin = () => {
  uni.navigateTo({ url: '/pages/doctor/auth/login' });
};
