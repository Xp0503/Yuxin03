  ...
  <view class="login-container">
    ...
    <view class="login-card">
      ...
      <view class="action-links">
        ...
        <text class="action-link" @click="navigateTo('/pages/doctor/auth/forgot')">忘记密码？</text>
		<text class="action-link" @click="navigateTo('/pages/auth/login')">用户端</text>
      </view>
    </view>
    ...
    <view class="register-section">
      <text>还没有账号？</text>
      <text class="register-link" @click="navigateTo('/pages/doctor/auth/register')">注册</text>
    </view>
...
const handleLogin = async () => {
  ...
  uni.showLoading({ title: '登录中...', mask: true })
   ...
   try {
     // 调用登录API
    const response = await loginDoctor(form.value.phone, form.value.password)
    doctorTokenStore.setDoctor(response)
     
     // 调试日志
     console.log('登录成功，医生信息:', response)
     console.log('Pinia 存储的医生信息:', doctorTokenStore.doctor)
     
     uni.showToast({ title: '登录成功', icon: 'success' })
 
     // 跳转到首页
     setTimeout(() => {
       uni.switchTab({ url: '/pages/doctor/index/index' })
     }, 1500)
   } catch (error) {
...
const switchToSMSLogin = () => {
  uni.navigateTo({
    url: '/pages/doctor/auth/sms-login'
  })
}
</script>
