...
const loadConversations = async () => {
  try {
    ...
    messages.value = res.map(conversation => ({
      ...
      sender: conversation.contact?.name || '未知联系人',
      avatar: conversation.contact?.avatar || '/static/msg/default-avatar.png',
      preview: conversation.lastMessage || '开始对话',
      time: conversation.lastMessageTime ? new Date(conversation.lastMessageTime) : new Date(),
...
const viewMessage = (msg) => {
  ...
  if (msg.id === 2) {
    uni.navigateTo({
      url: '/pages/msg/detail' 
    });
  } else {
    // 传递昵称和头像参数到详情页
     uni.navigateTo({
       url: `/pages/msg/msgdet?conversationId=${msg.conversationId}&contactName=${msg.sender}&contactAvatar=${msg.avatar}`
     });
  }
}
